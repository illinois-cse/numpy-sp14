...
It also plots the residuals with labels.
"""
...
dataplot.set_xlabel('d / mm')
dataplot.set_ylabel('F / N')

# Calculate the residuals.  (You can use LaTeX here as well; see http://wiki.scipy.org/Cookbook/Matplotlib/UsingTex)
resids = abs(F - np.polyval(a,d))
residsplot = plt.subplot(2,1,2)
residsplot.plot(d,resids,'bo')
residsplot.set_xlabel('d_exp / mm')
residsplot.set_ylabel('|F_exp - F_fit| / N')
